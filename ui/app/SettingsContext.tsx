...
export type GradeMetricEnabled = { [K in keyof GradeWeights]: boolean };

export const DEFAULT_GRADE_METRIC_ENABLED: GradeMetricEnabled = {
  apdex: true, conversion: true, errorRate: true, avgDuration: true,
  lcp: true, cls: true, inp: true, ttfb: true, sessions: false,
};

export type GradeMetricThresholds = { [K in keyof GradeWeights]: { good: number; poor: number } };

export const DEFAULT_GRADE_METRIC_THRESHOLDS: GradeMetricThresholds = {
  apdex:       { good: 0.94,  poor: 0.5   },
  conversion:  { good: 25,    poor: 1     },
  errorRate:   { good: 0.5,   poor: 5     },
  avgDuration: { good: 1500,  poor: 5000  },
  lcp:         { good: 2500,  poor: 4000  },
  cls:         { good: 0.1,   poor: 0.25  },
  inp:         { good: 200,   poor: 500   },
  ttfb:        { good: 800,   poor: 1800  },
  sessions:    { good: 10000, poor: 1000  },
};
...
const GRADE_METRIC_ENABLED_STATE_KEY = "uj-grade-metric-enabled";
const GRADE_METRIC_THRESHOLDS_STATE_KEY = "uj-grade-metric-thresholds";
...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const savedGradeMetricEnabled = useAppState({ key: GRADE_METRIC_ENABLED_STATE_KEY });
  const savedGradeMetricThresholds = useAppState({ key: GRADE_METRIC_THRESHOLDS_STATE_KEY });
