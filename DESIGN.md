...
- **Hotness Mode (user preference)**: Users can choose between `Shared` (default) and `Tab-specific` in Settings.
  - `Shared`: A single canonical hotness strip is published for all tabs from shared KPIs (traffic, conversion, Apdex, error rate, latency) plus active Davis problem overlap per bucket.
  - `Tab-specific`: Legacy per-tab formulas are restored (for example, funnel drop-off spike Z-scores, navigation FE+BE flow spikes, map traffic hotness).
...
- **Problem-aware diagnostics**: In Shared mode, bucket diagnosis includes active problem count and direct links to Gen3 problem cards for fast incident correlation.
- **Filter isolation (Tab-specific mode)**: Navigation Flow hotness intentionally ignores session/user filters (`selectedSessionId`, `navUserFilter`) so the fleet-wide spike pattern remains visible regardless of the current session scope.
